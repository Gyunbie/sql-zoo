...
def films_by_cast_size
  ...
  execute(<<-SQL)
    SELECT movies.title, COUNT(DISTINCT castings.actor_id) AS cast_count
    FROM movies
    INNER JOIN castings ON castings.movie_id = movies.id
    WHERE yr = 1978
    GROUP BY movies.id
    ORDER BY cast_count DESC, title ASC;
  SQL
end
...
def colleagues_of_garfunkel
  ...
  execute(<<-SQL)
    SELECT DISTINCT actor_list.name
    FROM (
      SELECT movies.*
      FROM movies
      INNER JOIN castings ON castings.movie_id = movies.id
    ...
    INNER JOIN (
      ...
      WHERE actors.name != 'Art Garfunkel'
    ) AS actor_list ON artgarf_movies.id = actor_list.movie_id;
  SQL
end
