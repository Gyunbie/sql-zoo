...
def alison_artist
  ...
  execute(<<-SQL)
    SELECT artist
    FROM albums
    INNER JOIN tracks ON albums.asin = tracks.album
...
def blur_songs
  ...
  execute(<<-SQL)
    SELECT song
    FROM tracks
    INNER JOIN albums ON tracks.album = albums.asin
...
def best_value
  ...
  # of tracks.
  execute(<<-SQL)
    SELECT albums.title, albums.price, COUNT(tracks.*)
    FROM albums
    INNER JOIN tracks ON albums.asin = tracks.album
    GROUP BY albums.asin
...
def rock_superstars
  ...
  execute(<<-SQL)
    ...
    FROM albums
    INNER JOIN styles ON albums.asin = styles.album
